#ifndef LECIES_UTIL_H
#define LECIES_UTIL_H
 ...
 * <c>x < y ? x : y</c>
 */
#define LECIES_MIN(x, y) (((x) < (y)) ? (x) : (y))

/**
 * <c>x > y ? x : y</c>
 */
#define LECIES_MAX(x, y) (((x) > (y)) ? (x) : (y))
 ...
 */
static inline size_t lecies_calc_aes_cbc_ciphertext_length(const size_t plaintext_length)
{
 ...
 */
static inline size_t lecies_calc_compression_bound(const size_t data_length)
{
 ...
 */
static inline size_t lecies_calc_output_buffer_needed_size(const size_t input_buffer_length, const size_t key_size)
{
    //     1    2    3          4    5
 ...
 * @return The min. buffer size for encrypting \p input_buffer_length bytes of data.
 */
static inline size_t lecies_curve25519_calc_output_buffer_needed_size(const size_t input_buffer_length)
{
    return lecies_calc_output_buffer_needed_size(input_buffer_length, LECIES_X25519_KEY_SIZE);
}
 ...
 * @return The min. buffer size for encrypting \p input_buffer_length bytes of data.
 */
static inline size_t lecies_curve448_calc_output_buffer_needed_size(const size_t input_buffer_length)
{
    return lecies_calc_output_buffer_needed_size(input_buffer_length, LECIES_X448_KEY_SIZE);
}
 ...
 */
static inline size_t lecies_calc_base64_length(const size_t data_length)
{
 ...
 */
LECIES_API int lecies_hexstr2bin(const char* hexstr, size_t hexstr_length, uint8_t* output, size_t output_size, size_t* output_length);
 ...
 */
LECIES_API int lecies_bin2hexstr(const uint8_t* bin, size_t bin_length, char* output, size_t output_size, size_t* output_length, int uppercase);

/**
 * Gets the current lecies version number as a human-readable string (e.g. <c>"2.1.2"</c>).
 * @return The stringified current lecies version number.
 */
LECIES_API char* lecies_get_version_str();

/**
 * Gets the current lecies version number as an unsigned integer (e.g. version <c>"2.1.2"</c> would return <c>212</c>).
 * @return The current lecies version number.
 */
LECIES_API uint64_t lecies_get_version_nr();

/**
 * Checks whether lecies fprintf is enabled (whether errors are fprintfed into stderr).
 * @return Whether errors are fprintfed into stderr or not (\c 1 for \c true ; \c 0 for \c false).
 */
LECIES_API int lecies_is_fprintf_enabled();
 ...
 * @return <c>0</c>
 */
static inline int lecies_printvoid(FILE* stream, const char* format, ...)
{
    return 0;
}

/** @private */
LECIES_API extern int (*lecies_fprintf_fptr)(FILE* stream, const char* format, ...);

/**
 * Enables lecies' use of fprintf().
 */
LECIES_API void lecies_enable_fprintf();

/**
 * Disables LECIES' use of fprintf().
 */
LECIES_API void lecies_disable_fprintf();

/** @private */
#define lecies_fprintf lecies_fprintf_fptr
 ...
 */
static inline unsigned long long int lecies_get_random_big_integer()
{
 ...
 */
LECIES_API void lecies_dev_urandom(uint8_t* output_buffer, size_t output_buffer_size);

/**
 * Free memory that was allocated by lecies. <p>
 * Wraps the <c>free()</c> function (mainly useful for C# interop).
 * @param mem The pointer to the memory to free.
 */
LECIES_API void lecies_free(void* mem);
